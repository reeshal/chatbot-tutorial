...
Choix d'implémentation :
  ...
  - la contrainte `ON DELETE CASCADE` laisse la base supprimer elle-même les
    messages d'une conversation effacée.

ChatSession dépend du CONTRAT, pas d'une implémentation précise : changer de
moteur de stockage ne demande AUCUNE modification de ChatSession.
"""
...
# `id` en IDENTITY : la base numérote les lignes elle-même.
# La clé étrangère ON DELETE CASCADE : supprimer une conversation efface
# automatiquement ses messages — la base s'en charge, pas de DELETE manuel.
